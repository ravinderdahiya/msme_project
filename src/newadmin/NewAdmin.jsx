...
import { createApiUrl, deleteApiUrl, getApiUrls, updateApiUrl } from "../services/apiUrlService.js";
...
function ApiUrlsPage() {
    ...
    return (
        <div className="new-admin-grid one-one">
            <Card title={editingId ? "Update API URL" : "Add API URL"} subtitle="Backend proxy and runtime URLs are controlled from database.">
                <form className="api-url-form-grid" onSubmit={handleSubmit}>
                    ...
                    <input value={form.name} onChange={(e) => setForm((prev) => ({ ...prev, name: e.target.value }))} placeholder="Display name" />
                    <input value={form.category} onChange={(e) => setForm((prev) => ({ ...prev, category: e.target.value }))} placeholder="Category (map/general)" />
                    <input value={form.url} onChange={(e) => setForm((prev) => ({ ...prev, url: e.target.value }))} placeholder="Full URL" />
                    <input value={form.description} onChange={(e) => setForm((prev) => ({ ...prev, description: e.target.value }))} placeholder="Description (optional)" />
                    ...
                    <div className="api-url-form-actions">
                        ...
                        {editingId ? (
                            <button type="button" className="new-admin-filter-btn" onClick={resetForm}>
                                Cancel
                            </button>
                        ) : null}
                    </div>
